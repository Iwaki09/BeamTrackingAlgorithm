...
def svm_exp(model, C=1, degree=1, coef0=1, gamma=1):
    ...
    ss = StandardScaler()
    X = ss.fit_transform(df[['speed', 'angle', 'angle_diff']].to_numpy())
    y = df['best'].to_numpy()
    ...
    [speed, angle, angle_diff] = ([7.5, 90, 0] - ss.mean_) / ss.scale_
    data = np.array([speed, angle, angle_diff]).reshape(1, -1)
    prediction1 = svm_model.predict(data)

    [speed, angle, angle_diff] = ([14, 111, 0.24] - ss.mean_) / ss.scale_
    data = np.array([speed, angle, angle_diff]).reshape(1, -1)
