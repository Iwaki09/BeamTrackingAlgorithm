function extended_main()
    ...
    output_name = 'curve_r150_75to90_oran';
    ...
    elseif strcmp(output_name(end-3:end), '4way')
        search_way = 4;
    elseif strcmp(output_name(end-3:end), 'oran') 
        search_way = 2; 
    end
    ...
    %     G   = rmd./(4*pi.*d).*sqrt(Ptx*Gtx*Grx*Gab);
        ...
        switch state
          ...
          case 'track'        
            ...
            % 10回に1回答え合わせをする。v_estとa_estを正しい値にする。(ここでの_estは1ステップ前の値なので、予想を更新するわけではなく、ここでの結果をもとにこの後で予想を立てる)
            if mod(nt, 10) == 0
              v_est = speed;
              a_est = 90 + atand((x_du - position(1)) / dy_k)
              if direct_or_not == 0
                dy_k = abs(y_du-position(2)) + 15.5;
              end
            end
