...
% sumocfgのpath
scenarioPath = '../curve_r60.sumocfg';
...
%     N_rx = wgn(n_du, 1, Np, 'linear', 'complex');
    ...
    switch state
      ...
      case 'wait'
        ...
        SNR_4 = real2db(abs(Wr*HG*Wt).^2/Np);
      ...
      case 'track'        
        ...
        if SNR_4 - SNR_04 < 0
          ...
          SNR_a4 = real2db(abs(Wr*HG*W_a).^2/Np);
          SNR_b4 = real2db(abs(Wr*HG*W_b).^2/Np);
          SNR_c4 = real2db(abs(Wr*HG*W_c).^2/Np);
          SNR_d4 = real2db(abs(Wr*HG*W_d).^2/Np);

          u = db2real(abs(SNR_4 - SNR_04));

          SNR_max = max([SNR_a4, SNR_b4, SNR_c4, SNR_d4]);
          if SNR_max == SNR_a4
              v_est = v_est + 2*3*u;
          elseif SNR_max == SNR_b4
              v_est = v_est + 3*u;
          elseif SNR_max == SNR_c4
              v_est = v_est - 3*u;
          else
              v_est = v_est - 2*3*u;
          end
    ...
    if f_plot == 1      
      ...
      switch state
        ...
        case 'track',
          ...
          plot(RU.ary.x, SNR_4, '.', 'Color', 'green', 'LineWidth', 1.5);
